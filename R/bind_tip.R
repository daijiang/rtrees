...
#' @param node_heights A named numeric vector of node hieghts of the tree, generated by \code{ape::branching.times()}. It is also optional if \code{tree} is specified; but required if \code{tree_tbl} is specified.
#' @return Either a phylogeny or a data frame, which can be then converted to a phylogeny later.
#' @examples 
#' \dontrun{
#' library(rtree)
#' bind_tip(tree_plant_GBOTB, "N70407", tip_label = "test_sp")
#' tree_plant_GBOTB_df = tidytree::as_tibble(tree_plant_GBOTB)
#' node_heights = ape::branching.times(tree_plant_GBOTB)
#' bind_tip(tree_tbl = tree_plant_GBOTB_df, where = "N70407", 
...
bind_tip = function(tree = NULL, where, tip_label,
  ...
  tree_tbl = dplyr::mutate(tree_tbl, isTip = (! node %in% parent))
  ...
  if(!tree_tbl_node$isTip) { # the target is a node
    ...
    if(new_node_above & !at_root){ # insert above the target location node
      ...
      tree_tbl_2 = dplyr::bind_rows(tree_tbl_2,
                                  tibble::tibble(parent = tree_tbl_2$node[tree_tbl_2$label == node_label],
                                                 node = max_offspring + 1,
                                                 branch.length = tree_tbl_node$branch.length * (1 - frac) + node_height,
                                                 label = tip_label))
      ...
      if(at_root){ # root
        ...
        tree_tbl_2$node = ifelse(tree_tbl_2$node > max_offspring, tree_tbl_2$node + 1, tree_tbl_2$node)
        tree_tbl_2 = dplyr::bind_rows(tree_tbl_2,
                                    tibble::tibble(parent = tree_tbl_2$node[tree_tbl_2$label == where],
                                                   node = max_offspring + 1,
