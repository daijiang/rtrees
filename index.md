...
though you should check the results. It is possible to just provide the
speices as a character vector for these taxon groups (or all genus in
your species list are presented in the phylogeny).
...
can also work with a set of posterior megatrees with the option to use
parallel computing for the whole process. The default number of cores to
be used will be the available number of cores minus 2 (so that users can
still perform other tasks on their computers at the same time). The
output will be a set of generated phylogenies with class `multiPhylo`;
the number of derived phylogenies will be the same as the input
megatrees. For this scenario, we can use exactly the same code described
above. For example, if we want to use the 100 randomly selected
posterior phylogenies that have ~32k fish species, we can add the
`fish_tree = "all-taxon`.
...
- The `tree` can be a user provided tree, if so, set
  `tree_by_user = TRUE`.
- See
  [`?get_tree`](https://daijiang.github.io/rtrees/reference/get_tree.md)
  for more details.
